...
### Лист "Master CV"
Источник резюме выбирается по Title вакансии.

Формат строк с резюме:

| Колонка A | Колонка B | Колонка C |
|-----------|-----------|-----------|
| `Master_CV_*` | ссылка/ID Google Doc с текстом резюме | маска поиска по Title |

Правила:
- Учитываются только строки, где колонка A начинается с `Master_CV_`.
- В колонке B можно хранить:
   - обычный URL Google Doc,
   - сырой Doc ID,
   - `HYPERLINK(...)`,
   - «красивую» гиперссылку (rich link) с текстом вместо URL.
- В колонке C задаётся подстрока для поиска в Title (без учёта регистра).
- Если совпадений несколько, используется первая (верхняя) строка.
- Если совпадений нет, используется строка с маской `*`.
- Если `*` строк несколько, используется первая (верхняя).
